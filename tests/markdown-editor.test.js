...
test('sidebar creates an editable markdown file without opening a file picker', () => {
  ...
  assert.match(page, /requestFullscreen \|\| target\.webkitRequestFullscreen \|\| target\.msRequestFullscreen/);
  assert.match(page, /switchMode\('preview'\);[\s\S]*requestAnimationFrame/);
  ...
  assert.match(page, /class: 'markdown-file-mode-switch markdown-mode-switch markdown-file-controls'/);
  assert.match(styles, /\.markdown-file-topbar \{[\s\S]*grid-template-columns: minmax\(0, 1fr\) auto;[\s\S]*min-height: 42px;[\s\S]*padding: 5px 10px/);
  assert.match(styles, /\.markdown-file-controls \{[\s\S]*justify-self: end;[\s\S]*gap: 4px/);
...
test('sidebar creates standalone HTML pages with preview-first source editing', () => {
  ...
  assert.match(htmlPage, /text: '全屏'/);
  assert.match(htmlPage, /requestFullscreen \|\| target\.webkitRequestFullscreen \|\| target\.msRequestFullscreen/);
  assert.match(htmlPage, /switchMode\('preview'\);[\s\S]*requestAnimationFrame/);
  assert.match(htmlPage, /html-file-controls[\s\S]*editButton[\s\S]*previewButton[\s\S]*openWindowButton[\s\S]*fullscreenButton/);
  assert.match(styles, /\.markdown-file-topbar \{[\s\S]*grid-template-columns: minmax\(0, 1fr\) auto;[\s\S]*min-height: 42px;[\s\S]*padding: 5px 10px/);
  assert.match(styles, /\.markdown-file-controls \{[\s\S]*justify-self: end;[\s\S]*gap: 4px/);
